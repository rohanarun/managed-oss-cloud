...
Set `control_plane_domain` and `apps_domain` in `terraform.tfvars` before production DNS. `control_plane_image` is required and accepts only an immutable GHCR `@sha256:` reference; use the digest published by the verified GitHub Actions build. Stripe, OAuth, signing, worker, gateway, session, and backup secrets belong in Google Secret Manager. Pin every corresponding `*_secret_version` input to a reviewed numeric version so a restart cannot silently consume a newly created `latest` value.
...
Paid quotas are pooled across the worker fleet. Starter is $7 monthly ($5 infrastructure + $2 fee) for 22 suite modules, 1.5 GB memory, 0.5 vCPU, 10 GB storage, and two service instances. Scale is $50 ($44.64 + $5.36) for 25 modules, 6 GB, 2 vCPU, 100 GB, and 12 instances. Fleet is $200 ($178.57 + $21.43) for all 27 modules, 24 GB, 8 vCPU, 500 GB, and 50 instances. These are quota envelopes rather than dedicated-machine claims. Suite-only customers reserve no per-customer application-container capacity. Each cloned upstream service gets its own storage reservation and can be placed on another worker while remaining in the same customer account.
