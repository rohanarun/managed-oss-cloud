...
die() {
  printf 'Worker reboot reapply refused: %s\n' "$*" >&2
  exit 1
}
...
' "$manifest" >/dev/null || die "release manifest does not preserve the safe v0.4.0 worker mode"
...
[[ "$(env_value CONTROL_PLANE_IMAGE)" == "$expected_image" ]] || die "current image is not the exact release digest"
...
if [[ "$preflight_only" == "true" ]]; then
  for service in agent caddy; do
    container_id="$(cd /opt/managed-oss/config && docker-compose ps -q "$service")"
    [[ -n "$container_id" && "$(docker inspect --format '{{.State.Status}}' "$container_id")" == "running" ]] || die "current worker container state is not running"
  done
  ...
  jq -cn \
    ...
    '{ok: true, role: $role, runtimeReady: true, hmacAbsent: true, runtimeSha256: $runtimeSha256, releaseManifestSha256: $manifestSha256}'
  exit 0
fi
...
proof="/opt/managed-oss/provenance/reboot-${image_hex}-${source_commit}-$(date -u +%Y%m%dT%H%M%SZ).json"
...
metadata_proof="/opt/managed-oss/security/worker-metadata-${image_hex}-$(date -u +%Y%m%dT%H%M%SZ).json"
