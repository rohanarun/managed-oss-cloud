...
All 37 first-party product distributions also have independent public MIT repositories. The v0.3.1 release gives each repository a product-specific workspace, guided forms for every declared action, server-filtered and paginated record summaries, detail-only payload reads, grouped workflows, a screenshot captured against the real authenticated shared backend, a scoped HTTP client, CLI, stdio MCP server, digest-pinned Dockerfile, 12 repository tests, and a verified functional tutorial. Every tutorial runs the product's real browser workflow, requires an HTTP 200 mutation and durable-record readback, reopens the exact record in the UI, and publishes Gemini-generated burned-in steps, SubRip subtitles, and a hash-bound proof receipt. A shared acceptance test also runs every product's primary workflow through the real control-plane backend and replays and observes it through that repository's client, CLI, and MCP server. They share tenant data only through this control plane's authenticated API; none contains database credentials or a competing migration owner. Exact accepted-v1 and external-provider stop lines are recorded in the [product acceptance matrix](docs/product-v1-acceptance.md).
