...
import type { AccountUser, BackupRecord, CatalogApp, DashboardData, Installation, Quote } from "../shared/types";
...
interface PublicConfig {
  ...
  plans: Array<{ id: string; label: string; memoryMb: number; cpu: number; storageGb: number; maxServices: number; infrastructureMonthlyCents: number; monthlyCents: number }>;
}
...
function Dashboard({ config, catalog, user, navigate, onLogout }: { config: PublicConfig; catalog: CatalogApp[]; user: AccountUser; navigate: (path: string) => void; onLogout: () => void }) {
  ...
  async function upgrade(plan: string) { if (!active) return; try { const result = await api<{ installation: Installation }>(`/api/installations/${active.id}/upgrade`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plan }) }); setActive(result.installation); setNotice(`Upgrade to ${plan} saved and reconciled with billing when required.`); await load(); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not plan upgrade."); } }
  async function cloneService() { if (!active) return; try { await api(`/api/installations/${active.id}/applications`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ appId: cloneAppId }) }); const refreshed = await api<DashboardData>("/api/dashboard"); setData(refreshed); setActive(refreshed.installations.find((item) => item.id === active.id) ?? active); setNotice("Service clone reserved. It will receive its own hostname, containers, and persistent storage."); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not clone service."); } }
  ...
  async function manageApplication(applicationInstanceId: string, action: "start" | "stop" | "backup" | "restore", objectName?: string) { if (!active) return; try { await api(`/api/installations/${active.id}/actions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, applicationInstanceId, objectName }) }); setNotice(`${action[0].toUpperCase() + action.slice(1)} queued for this application.`); if (action === "backup" || action === "restore") setTimeout(() => { void api<BackupRecord[]>(`/api/installations/${active.id}/backups`).then(setBackups); }, 2500); await load(); } catch (error) { setNotice(error instanceof Error ? error.message : `Could not ${action} application.`); } }
  ...
  const usedMemory = active?.applications?.reduce((sum, app) => sum + app.memoryReservationMb, 0) ?? 0;
  const usedStorage = active?.applications?.reduce((sum, app) => sum + app.storageReservationGb, 0) ?? 0;
  const currentPlan = active ? config.plans.find((plan) => plan.id === active.plan) : undefined;
  useEffect(() => { if (!active) return setBackups([]); void api<BackupRecord[]>(`/api/installations/${active.id}/backups`).then(setBackups).catch(() => setBackups([])); }, [active?.id]);
  return <main className="dashboard-shell"><aside className="dashboard-sidebar"><button className="dashboard-brand" onClick={() => navigate("/")}><Brand name={config.productName} /></button><nav>{[["overview", Gauge, "Overview"], ["catalog", Storefront, "Catalogue"], ["servers", HardDrives, "Servers"], ["domains", Globe, "Domains"], ["billing", CreditCard, "Billing"]].map(([id, Icon, label]) => <button key={String(id)} className={section === id ? "active" : ""} onClick={() => go(String(id))}><Icon />{String(label)}</button>)}</nav><div className="account-card"><span>{user.displayName.slice(0, 1).toUpperCase()}</span><div><strong>{user.displayName}</strong><small>{user.email}</small></div><button onClick={logout}><SignOut /></button></div></aside><section className="dashboard-main"><header><div><span>Workspace control plane</span><h1>{section === "overview" ? `Good to see you, ${user.displayName.split(" ")[0]}.` : section[0].toUpperCase() + section.slice(1)}</h1></div><button className="button primary" onClick={() => go("catalog")}><Plus /> Install software</button></header>{notice && <div className="notice">{notice}<button onClick={() => setNotice("")}><X /></button></div>}
    ...
    {section === "servers" && <div className="management-grid"><ServerTable installations={installations} catalog={catalog} onSelect={setActive} />{active ? <div className="server-detail"><div><HardDrives /><span>{active.state}</span></div><h2>{active.name}</h2><p>{active.failureReason || active.hostname}</p>{active.applications?.map((application) => { const latestBackup = backups.find((backup) => backup.applicationInstanceId === application.id); return <div className="managed-app" key={application.id}><div><span>{catalog.find((item) => item.id === application.appId)?.name ?? application.appId}</span><small>{application.hostname}</small></div><strong>{application.state}</strong><div className="managed-app-actions">{application.state === "live" ? <><a href={`https://${application.hostname}`} target="_blank" rel="noreferrer">Open <ArrowRight /></a><button onClick={() => manageApplication(application.id, "backup")}>Back up</button><button onClick={() => manageApplication(application.id, "stop")}>Stop</button></> : <button onClick={() => manageApplication(application.id, "start")}>Start</button>}{latestBackup && <button onClick={() => manageApplication(application.id, "restore", latestBackup.objectName)}>Restore latest</button>}</div></div>})}<div className="clone-service"><select value={cloneAppId} onChange={(event) => setCloneAppId(event.target.value)}>{catalog.filter((app) => app.status === "ready").map((app) => <option key={app.id} value={app.id}>Clone {app.name}</option>)}</select><button onClick={cloneService}><Plus /> Clone service</button></div><div className="capacity-bar"><span>{usedMemory} of {currentPlan?.memoryMb ?? 0} MB · {usedStorage} of {currentPlan?.storageGb ?? 0} GB · {active.applications?.length ?? 0} of {currentPlan?.maxServices ?? 0} services</span><i><b style={{ width: `${Math.min(100, (usedMemory / (currentPlan?.memoryMb || 1)) * 100)}%` }} /></i></div><h3>Upgrade capacity</h3><div className="plan-buttons">{config.plans.map((plan) => <button key={plan.id} disabled={plan.id === active.plan} onClick={() => upgrade(plan.id)}><strong>{plan.label} · {money(plan.monthlyCents)}</strong><span>{plan.memoryMb / 1024} GB RAM · {plan.storageGb} GB storage · {plan.maxServices} services</span></button>)}</div></div> : <div className="empty-panel"><HardDrives /><p>Select a server to manage capacity.</p></div>}</div>}
