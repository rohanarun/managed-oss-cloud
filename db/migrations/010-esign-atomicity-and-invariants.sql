...
CREATE OR REPLACE FUNCTION managed_oss_complete_suite_ai_action_v2(
  p_action_id UUID,
  p_status TEXT,
  p_result JSONB,
  p_last_error TEXT
...
BEGIN
  ...
  IF v_action.context->'resultContract'->>'version' IS DISTINCT FROM 'esign-ai-result.v1' THEN
    RETURN public.managed_oss_complete_suite_ai_action(p_action_id,p_status,p_result,p_last_error);
  END IF;
  ...
  IF COALESCE(v_action.context->>'aiAuditRecordId','') !~ '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$' THEN
    RAISE EXCEPTION 'e-signature AI action has no valid audit record';
  END IF;
  ...
  IF p_status='completed' THEN
    IF EXISTS (
      ...
      WHERE key NOT IN ('proposals','confidence','assumptions','reviewStatus','approvalRequired','model')
    )
      ...
      RAISE EXCEPTION 'e-signature AI completion violates its trusted result contract';
    END IF;
    ...
    IF EXISTS (
      ...
      WHERE pg_catalog.jsonb_typeof(value) IS DISTINCT FROM 'object'
        OR EXISTS (
          ...
          WHERE key NOT IN ('proposalId','kind','text','citations','rationale','riskFlags')
        )
  ...
  IF p_status='completed' THEN
    ...
    WHERE id=v_audit_id
      ...
      AND module_id='esign'
      AND record_type='esign-ai-request-audit'
    ...
    IF v_updated IS NULL THEN RAISE EXCEPTION 'e-signature AI audit record is missing or stale'; END IF;
  ELSE
    UPDATE public.suite_records
...
ALTER FUNCTION managed_oss_complete_suite_ai_action_v2(UUID,TEXT,JSONB,TEXT) OWNER TO managed_oss_suite_owner;
REVOKE ALL ON FUNCTION managed_oss_complete_suite_ai_action_v2(UUID,TEXT,JSONB,TEXT) FROM PUBLIC,managed_oss_runtime;
GRANT EXECUTE ON FUNCTION managed_oss_complete_suite_ai_action_v2(UUID,TEXT,JSONB,TEXT) TO managed_oss_ai;
